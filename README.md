...
```bash
$ helm install --name my-release https://github.com/getupcloud/fluent-bit-out-syslog.git
```
...
```bash
$ helm install --name my-release https://github.com/getupcloud/fluent-bit-out-syslog.git --set on_minikube=true
```
...
| -----------------------    | ---------------------------------- | ----------------------- |
| **Backend Selection**      |
| `backend.type`             | Set the backend to which Fluent-Bit should flush the information it gathers | `syslog` |
| **Syslog Backend**        |
| `backend.syslog.host`     | Target host where Fluent-Bit or Fluentd are listening for Forward messages | `fluentd` |
| `backend.syslog.port`     | TCP Port of the target service | `24284` |
